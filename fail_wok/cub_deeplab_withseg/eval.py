...
def main():
    ...
    from pspnet import PSPNet
    model = PSPNet(backbone = args.backbone, layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, use_softmax=True, use_aux=True, pretrained=False, syncbn=False).cuda()
...
def validate(val_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w, scales):
    ...
    for i, (input, _, att) in enumerate(val_loader):
        ...
        result, output = model.forward_cam(input_var)
