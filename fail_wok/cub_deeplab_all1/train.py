...
def main():
    ...
    optimizer = torch.optim.SGD(
      ...
      #   {'params': model.ppm.parameters(), 'lr': args.base_lr * 10},
         ...
         {'params': model.result.parameters(), 'lr': args.base_lr * 10}],
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandRotate([args.rotate_min, args.rotate_max], padding=mean, ignore_label=0),
        transforms.RandomGaussianBlur(),
        transforms.RandomHorizontalFlip(),
        transforms.Crop([args.crop_h, args.crop_w], crop_type='rand', padding=mean, ignore_label=0),
        transforms.ToTensor(),
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        loss_train = train(train_loader, model, criterion, optimizer, epoch, args.zoom_factor, args.batch_size, args.aux_weight)
        if rank == 0:
...
def train(train_loader, model, criterion, optimizer, epoch, zoom_factor, batch_size, aux_weight):
    ...
    for i, (input, target, atts) in enumerate(train_loader):
        ...
        output, output_bak = model(input_var)  #, aux
        #print(output_bak.size())
   ...
   #         target = F.upsample(target.unsqueeze(1).float(), size=(h, w), mode='bilinear').squeeze(1).long()
        ...
        h = int((target.size()[1] - 1) / 8 + 1)
        w = int((target.size()[2] - 1) / 8 + 1)
        target = F.upsample(target.unsqueeze(1).float(), size=(h, w), mode='bilinear').squeeze(1)
        #print (target.max(), target.min())
        target = target.data.cuda(async=True)
        target_new = ((output_bak.mean(dim=1).data * target).mean(1).mean(1).unsqueeze(1).unsqueeze(2).expand(target.size())*target).unsqueeze(1).expand(output_bak.size())
        target = (output_bak.sum(dim=1).data * target).mean(1).mean(1).unsqueeze(1).unsqueeze(2).expand(target.size())*target
        
        print((output_bak.sum(dim=1).data * target).mean(1).mean(1))
        target_var = torch.autograd.Variable(target.float())
        target_new_var = torch.autograd.Variable(target_new.float())
        atts_var = torch.autograd.Variable(atts.float().cuda())
        main_loss = criterion_mse(output, atts_var) / world_size + criterion_mse(output_bak.sum(dim=1), target_var) / (world_size*50) + criterion_mse(output_bak, target_new_var)/ (world_size*50)
