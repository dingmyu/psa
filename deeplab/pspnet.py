...
class PSPNet(nn.Module):
    ...
    #            m.stride = (1, 1)
        ...
        self.cls1 = nn.Sequential(
            ...
            nn.Conv2d(256, 20, kernel_size=1, bias=False),
        )
    ...
    def forward_cam(self, x):
        ...
        if self.use_ppm:
            x1,x3,x6,x9 = self.ppm(x)
        x1 = F.conv2d(x1, self.cls1[1].weight)
        x3 = F.conv2d(x3, self.cls3[1].weight)
        x6 = F.conv2d(x6, self.cls6[1].weight)
        x9 = F.conv2d(x9, self.cls9[1].weight)
        x1 = F.relu(x1)
        x3 = F.relu(x3)
        x6 = F.relu(x6)
        x9 = F.relu(x9)
